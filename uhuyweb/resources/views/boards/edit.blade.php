...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="text-center mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Board</h1>
        <p class="text-gray-600">Perbarui informasi board Anda</p>
    </div>

    <form action="{{ route('boards.update', $board) }}" method="POST" class="bg-white rounded-2xl shadow-lg overflow-hidden">
        @csrf
        @method('PUT')
        <div class="p-8 space-y-6">
            ...
            <div>
                ...
                @enderror
                <p class="mt-1 text-xs text-gray-500" id="name-counter">{{ strlen(old('name', $board->name)) }} / 255 karakter</p>
            </div>
            ...
            <div>
                ...
                <textarea name="description"
                          ...
                          maxlength="1000"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200 resize-none @error('description') border-blue-500 @enderror">{{ old('description', $board->description) }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-600">
                        <i class="fas fa-exclamation-circle mr-1"></i>
                        {{ $message }}
                ...
                @enderror
                <p class="mt-1 text-xs text-gray-500" id="desc-counter">{{ strlen(old('description', $board->description)) }} / 1000 karakter</p>
            </div>
                ...
                <div class="space-y-3">
                    <label class="flex items-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors duration-200">
                        <input type="radio"
                               name="is_private"
                               value="0"
                               {{ old('is_private', $board->is_private ? '1' : '0') == '0' ? 'checked' : '' }}
                    ...
                    <label class="flex items-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors duration-200">
                        <input type="radio"
                               name="is_private"
                               value="1"
                               {{ old('is_private', $board->is_private ? '1' : '0') == '1' ? 'checked' : '' }}
                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                        <div class="ml-4 flex-1">
                            <div class="flex items-center">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm text-gray-600">
                    <div>
                        <p class="font-medium">Total Pin:</p>
                        <p>{{ $board->pins()->count() }} pin</p>
                    </div>
                    <div>
                        <p class="font-medium">Dibuat:</p>
                        <p>{{ $board->created_at->format('d M Y, H:i') }}</p>
                    </div>
                    <div>
                        <p class="font-medium">Terakhir diubah:</p>
                        <p>{{ $board->updated_at->format('d M Y, H:i') }}</p>
                    </div>
                    <div>
            ...
            <div class="bg-gray-50 rounded-lg p-6">
                ...
                <div class="bg-white rounded-lg shadow-sm p-4">
                    <div class="flex items-start space-x-4">
                        ...
                        <div class="flex-1">
                            <h4 class="font-bold text-gray-900" id="preview-name">{{ $board->name }}</h4>
                            <p class="text-sm text-gray-600 mt-1" id="preview-desc">{{ $board->description ?: 'Deskripsi board akan muncul di sini' }}</p>
                            <div class="flex items-center mt-2 text-xs text-gray-500">
                                <i class="fas fa-thumbtack mr-1"></i>
                                <span>{{ $board->pins()->count() }} pin</span>
                                <span class="mx-2">•</span>
                                <i id="preview-privacy-icon" class="fas {{ $board->is_private ? 'fa-lock text-gray-500' : 'fa-globe text-green-500' }} mr-1"></i>
                                <span id="preview-privacy-text">{{ $board->is_private ? 'Private' : 'Publik' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
...
document.getElementById('name').addEventListener('input', function() {
    ...
    preview.textContent = this.value || '{{ $board->name }}';
...
document.getElementById('description').addEventListener('input', function() {
    ...
    preview.textContent = this.value || '{{ $board->description ?: "Deskripsi board akan muncul di sini" }}';
...
document.querySelector('form').addEventListener('submit', function(e) {
    ...
    submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i><span>Mengupdate Board...</span>';
});
